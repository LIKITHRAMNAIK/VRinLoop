...
function LoanUsersPage() {
  ...
  loanData.forEach((tx) => {
    ...
    const user = groupedUsers[tx.person_name];

    user.totalLoans += 1;

    user.totalBorrowed += tx.principal_amount || 0;

    user.totalRemaining += (tx.remaining_emi || 0) * (tx.emi_amount || 0);

    user.totalEmis += tx.loan_duration || 0;

    user.paidEmis += tx.completed_emi || 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Advanced EMI tracking, repayment analytics and loan health monitoring
        </p>
      </div>
      ...
      >
        {users.map((user) => {
          const progress =
            user.totalEmis === 0
              ? 0
              : Math.round((user.paidEmis / user.totalEmis) * 100);
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {formatCurrency(user.totalRemaining)}
                </h1>
              </div>
            </div>
          );
