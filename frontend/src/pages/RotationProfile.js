...
function RotationProfile({ data, refresh }) {
  ...
  const renderCard = (tx) => {
    ...
    const latestDueDate =
      tx.status === "paid" ? tx.final_due_date || tx.due_date : tx.due_date;
    ...
    const isOverdue =
      dueDate < today && tx.status !== "paid" && tx.status !== "extended";
    ...
    const finalInterest =
      tx.status === "paid"
        ...
        : (() => {
            let interest = Number(tx.base_interest || 0);

            tx.extensions?.forEach((ext) => {
              if (ext.interest_paid) {
                interest = Number(ext.extra_interest || 0);
              } else {
                interest += Number(ext.extra_interest || 0);
              }
            });
      ...
      {detailsPopup && (
        ...
        >
          ...
          >
            ...
            >
              <p>
                <b>Status:</b> {detailsPopup.status}
              </p>
              ...
              <p>
                <b>Final Interest:</b>
                <br />₹{detailsPopup.final_interest || 0}
              </p>

              <p>
                <b>Final Total:</b>
                <br />₹{detailsPopup.final_total || 0}
              </p>
            </div>
              ...
              {detailsPopup.final_extensions?.length > 0 ? (
                detailsPopup.final_extensions.map((ext, index) => (
                  ...
                  >
                    ...
                    <p>
                      <b>Extra Interest:</b>
                      <br />₹{ext.extra_interest || 0}
                    </p>

                    <p>
                      <b>Interest Paid:</b>
                      <br />
                      {ext.interest_paid ? "YES" : "NO"}
                    </p>
                  </div>
                ))
              ) : (
      ...
      {earlyPayPopup && (
        ...
        >
          ...
          >
            ...
            <p>
              <b>Principal:</b> {formatCurrency(earlyPayPopup.principal_amount)}
            </p>
            ...
            {earlyPayPopup.extensions?.length > 0 && (
              ...
              >
                <b>Extended Due:</b>{" "}
                {new Date(
                  earlyPayPopup.extensions[earlyPayPopup.extensions.length - 1]
                    .new_due_date,
                ).toDateString()}
              </p>
            )}
