...
function LoanProfile({ data, refresh }) {
  ...
  const [emiCounts, setEmiCounts] = useState({});
  const [emiPenalties, setEmiPenalties] = useState({});
...
refresh();
  ...
  const renderCard = (tx) => {
    ...
    tx.emi_history?.forEach((emi) => {
      totalPaid += emi.amount || 0;

      totalPenalty += emi.penalty_amount || 0;

      if (emi.status === "late") {
        latePayments += 1;
      } else if (emi.status === "advance") {
        advancePayments += 1;
      } else {
        onTimePayments += 1;
      }
    ...
    const totalEmis = tx.emi_history?.length || 0;
    ...
    const paymentScore =
      totalEmis === 0
        ...
        : Math.max(
            ...
            Math.round(((onTimePayments + advancePayments) / totalEmis) * 100),
          );

    const paidMonths = tx.completed_emi || 0;

    const totalMonths = tx.loan_duration || 1;

    const progress = Math.min((paidMonths / totalMonths) * 100, 100);

    const remainingMonths = tx.remaining_emi || 0;
    ...
    const isUpcoming = diffDays < 0 && Math.abs(diffDays) <= 3;
    ...
    return (
      <div
        ...
        style={{
          padding: 15,
          borderRadius: 12,
          background: isOverdue ? "#ffe5e5" : "#ede7f6",

          border: isOverdue ? "2px solid #f44336" : "1px solid #d1c4e9",
        }}
      >
        <div
          style={{
        ...
        <p>
          <b>Principal:</b> {formatCurrency(tx.principal_amount)}
        </p>

        <p>
          <b>EMI:</b> {formatCurrency(tx.emi_amount || 0)}
        </p>

        <p>
          <b>Duration:</b> {tx.loan_duration || 0} months
        </p>
        ...
        <p>
          <b>Remaining:</b> {remainingMonths}
        </p>
        <p>
          <b>Balance Amount:</b>{" "}
          {formatCurrency(remainingMonths * (tx.emi_amount || 0))}
        </p>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          >
            <b>Due Date:</b> {new Date(tx.due_date).toDateString()}
          </p>
        </div>
          ...
          >
            <div
              style={{
                width: `${progress}%`,
                height: "100%",
                background: progress === 100 ? "#4CAF50" : "#7E57C2",
        ...
        {tx.status !== "paid" && (
          ...
          >
            ...
            >
              {[...Array(Math.min(tx.remaining_emi, 6))].map((_, index) => (
  ...
  const handleDeleteLoan = async () => {
    try {
      await API.delete(`/delete/${deleteLoanId}`);
  ...
  {renderCard(tx)}
          ...
          >
            ...
            >
              <button
                onClick={async () => {
                  try {
                    await API.put(`/loan-history-date`, {
      ...
      {openHistory &&
        (() => {
          const analytics = openHistory?.emi_history || [];
          ...
          const paymentScore =
            analytics.length === 0
              ? 100
              : Math.round(
                  ((onTimePayments + advancePayments) / analytics.length) * 100,
                );

          const nextEmiDate = new Date(openHistory.due_date);
          ...
          const nextEmiDiff = Math.ceil(
            (nextEmiDate - currentDate) / (1000 * 60 * 60 * 24),
          );

          const isNextOverdue = nextEmiDiff < 0;

          const isDueToday = nextEmiDiff === 0;

          const riskLevel =
            paymentScore >= 80 ? "LOW" : paymentScore >= 50 ? "MEDIUM" : "HIGH";
              ...
              >
                <div
                  style={{
                    ...
                    color: "white",
                    marginBottom: 20,
                    boxShadow: "0 10px 25px rgba(94,53,177,0.25)",
                  }}
                >
                  {/* HEADER */}
                  <div
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {formatCurrency(openHistory.principal_amount)}
                      </h3>
                    </div>
                ...
                {openHistory.emi_history?.length > 0 ? (
                  openHistory.emi_history.map((emi, index) => (
                    ...
                    >
                      ...
                      <p>Amount: {formatCurrency(emi.amount)}</p>
                      {emi.penalty_amount > 0 && (
                        ...
                        >
                          Penalty: {formatCurrency(emi.penalty_amount)}
                        </p>
                      )}
                      ...
                      <p>
                        Paid Date:{" "}
                        {emi.paid_date
                          ? new Date(emi.paid_date).toDateString()
                          : "N/A"}
                      </p>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setEditDate(
                            emi.paid_date ? emi.paid_date.substring(0, 10) : "",
                          );
                        }}
                        style={{
                          background: "#5E35B1",
                ...
                <button
                  onClick={() => setDeleteLoanId(openHistory._id)}
                  style={{
                    width: "100%",
                    marginTop: 20,
                    padding: 12,
      ...
      {deleteLoanId && (
        ...
        >
          ...
          >
            ...
            >
              This loan and all EMI history will be permanently removed.
            </p>
