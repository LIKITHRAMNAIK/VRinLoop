  ...
  try {
    ...
    transactions.forEach((tx) => {
      ...
      if (isActive) {
        ...
        const finalInterest =
          tx.status === "paid"
            ? Number(tx.final_interest || totalInterest)
            : totalInterest;

        const total =
          tx.status === "paid"
            ? Number(tx.final_total || tx.principal_amount + finalInterest)
...
exports.payLoanEmi = async (req, res) => {
  try {
    ...
    if (tx.remaining_emi === undefined) {
      tx.remaining_emi = Number(tx.loan_duration || 0);
    }
    ...
    const emiCount = Number(req.body.emiCount || 1);

    const penaltyAmount = Number(req.body.penaltyAmount || 0);

    // SAFE LIMIT
    const safeCount = Math.min(emiCount, tx.remaining_emi);

    // ================= EMI LOOP =================

    for (let i = 0; i < safeCount; i++) {
      const currentMonth = tx.completed_emi + 1;

      const paidDate = new Date();

      // ================= EMI STATUS =================

      const dueDate = new Date(tx.start_date);

      // actual EMI month
      dueDate.setMonth(dueDate.getMonth() + tx.completed_emi);
      ...
      const comparePaidDate = new Date(paidDate);

      const compareDueDate = new Date(dueDate);

      comparePaidDate.setHours(0, 0, 0, 0);

      compareDueDate.setHours(0, 0, 0, 0);

      if (comparePaidDate > compareDueDate) {
        emiStatus = "late";

        const timeDifference =
          comparePaidDate.getTime() - compareDueDate.getTime();

        lateDays = Math.round(timeDifference / (1000 * 60 * 60 * 24));
      }
    ...
    if (tx.remaining_emi > 0) {
      ...
      currentDueDate.setMonth(currentDueDate.getMonth() + safeCount);
...
exports.markAsPaid = async (req, res) => {
  try {
    ...
    if (tx.transaction_type === "normal") {
      ...
      tx.paid_amount = (tx.paid_amount || 0) + amount;
    ...
    if (tx.status === "paid") {
      totalInterest = tx.final_interest || totalInterest;
    }
    ...
    if (req.body.earlyPay) {
      ...
      tx.early_paid_interest = Number(req.body.newInterest || 0);

      totalInterest = tx.early_paid_interest;
    }

    const finalTotal = tx.principal_amount + totalInterest;
...
exports.updateTransaction = async (req, res) => {
  try {
    ...
    // 🔥 BASIC UPDATE
    tx.principal_amount = req.body.principal_amount;

    tx.start_date = req.body.start_date;

    tx.loan_duration = req.body.loan_duration || tx.loan_duration;

    tx.emi_amount = req.body.emi_amount || tx.emi_amount;

    tx.interest_type = req.body.interest_type || tx.interest_type;

    if (tx.extensions && tx.extensions.length > 0) {
      // 🔥 update latest extension date
      tx.extensions[tx.extensions.length - 1].new_due_date = req.body.due_date;
...
exports.addTransaction = async (req, res) => {
  try {
    const { loan_mode, already_paid_months, last_paid_date } = req.body;
    ...
    if (req.body.transaction_type === "loan") {
      if (loan_mode === "existing") {
        ...
        remainingEmi = Number(req.body.loan_duration) - completedEmi;
      } else {
        completedEmi = 0;

        remainingEmi = Number(req.body.loan_duration);
      }
    }

    const emiHistory = [];

    if (req.body.transaction_type === "loan" && loan_mode === "existing") {
      for (let i = 0; i < Number(already_paid_months || 0); i++) {
        const paidDate = new Date(req.body.start_date);

        paidDate.setMonth(paidDate.getMonth() + i);

        // LAST EMI DATE
        if (i === Number(already_paid_months) - 1 && last_paid_date) {
          paidDate.setTime(new Date(last_paid_date).getTime());
        }
    ...
    const remainingBalance = Number(req.body.principal_amount) - totalPaid;
    ...
    if (
      ...
      let totalInterest = Number(req.body.base_interest || 0);
      ...
      finalTotal = Number(req.body.principal_amount) + totalInterest;
    }
    console.log({
      completedEmi,
      remainingEmi,
      loanStatus: Number(remainingEmi) <= 0 ? "paid" : "pending",
    });
    ...
    const transaction = new Transaction({
      ...
      last_payment_date:
        req.body.installments?.length > 0
          ? req.body.installments[req.body.installments.length - 1].date
          : null,
      ...
      base_interest: Number(req.body.base_interest),
      ...
      loan_duration: Number(req.body.loan_duration || 0),

      emi_amount: Number(req.body.emi_amount || 0),

      interest_type: req.body.interest_type || "flat",

      completed_emi: completedEmi,
      ...
      final_due_date: req.body.due_date,

      final_extensions: (req.body.extensions || []).map((ext) => ({
        old_due_date: ext.old_due_date,

        new_due_date: ext.new_due_date,

        extra_interest: Number(ext.extra_interest || 0),

        interest_paid: ext.interest_paid || false,
      })),
        ...
        req.body.transaction_type === "rotation"
          ...
          : // ================= LOAN =================
            ...
            : // ================= NORMAL =================
              ...
              ? totalPaid >= Number(req.body.principal_amount)
                ? "paid"
                : "pending"
              : "pending",

      paid_amount: req.body.transaction_type === "normal" ? totalPaid : 0,
    });
...
const updateLoanHistoryDate = async (req, res) => {
  try {
    const { txId, emiIndex, paid_date } = req.body;
    ...
    if (!transaction.emi_history || !transaction.emi_history[emiIndex]) {
      return res.status(404).json({
        message: "EMI not found",
      });
    }

    const emi = transaction.emi_history[emiIndex];

    emi.paid_date = paid_date;

    // ================= RECALCULATE STATUS =================

    const comparePaidDate = new Date(paid_date);

    comparePaidDate.setHours(0, 0, 0, 0);

    const dueDate = new Date(transaction.start_date);

    dueDate.setMonth(dueDate.getMonth() + emiIndex);
    ...
    if (comparePaidDate > dueDate) {
      ...
      lateDays = Math.round(
        (comparePaidDate - dueDate) / (1000 * 60 * 60 * 24),
      );
    }
